---
title: 'STAA 577: Laboratory Five </br> Cross validation (`tidyverse`)'
...
purrr::map_dbl(1:10, function(.x) {
  fit <- stats::glm(mpg ~ stats::poly(horsepower, degree = .x, raw = TRUE), data = Auto)
...
A salutary lesson on how `R` thinks about polynomials
...
# Comments
## Major
### The first LOOCV loop over the polynomial power mysteriously crashes on the second time through the loop. Lines 93--100. The syntax is identical to lines 122--126 and the loop completes successfully if I introduce K=10 say to the boot::cv.glm command. 

## FIXED BY SJT:  You do not want to use "raw" polynomials!

* I could not reproduce this error, the poly(raw = TRUE) is the same as poly(raw = FALSE)
 -->
