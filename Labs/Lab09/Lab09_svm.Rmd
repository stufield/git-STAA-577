...
lie outside the margin. For example, point 14 lies outside the margin
and its coefficient has magnitude equal to the cost.
Points 1, 7, and 17 have a coefficient with magnitude less than the
cost, and lie on the margin.
...
<!-- # SJT comments
## Major
* I do not understand the coefs or rho output from svm. I want to drop these.
  The only sensible IMHO thing the coefs can be are the coefficients 
  \hat\alpha that appear in equation (12.17), page 421 of ESL 2nd edition.
  This comes from expressing the SVM as a quadrative programming problem
  and considering the Kuhn-Tucker optimality conditions. 
  + This is up to you, feel free to drop if needed.
* I think I understand the ROC curves now, but I want to discuss. 
  + No problem. Love ROCs, they're a pretty integral part of what we do in BI.
## Minor
* What is the NULL trick in ggplot2?
  + if you add an extra line at the end `+` \n NULL, then you can comment out
    lines above and it will not break because there is a missing `+` sign
    at the bottom. For debugging while you're plotting. I've learned this trick
    and now I use it so much I just do it every time I make a ggplot.
* I don't understand your comment about a diabolical edge case
  +
* Error creating notebook. See line 298
  + Looks like the `yardstick` package changed the format of what 
    comes out of `yardstick::accuracy()`, and probably the other similar functions,
    so we were indexing something what wasn't there. Looks like you were on
    the right track. Just make sure you have yardstick >=0.0.2 and dplyr >= 0.7.8
    It should work (this had me out of commission for about 1h trying to figure it out)
 -->
