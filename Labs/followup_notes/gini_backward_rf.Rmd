...
A convenient property of the Gini impurity metric `G` is that it allows one to calculate a *theoretical maximum* that is based **solely** on the initial class distribution:
```{r}
...
## Gini Example
For a two-class problem, the `G` is maximized at equal class proportions, and minimized for either homogeneous case. **Note:** the goal is to *minimize* `G`:
```{r}
...
From the `gini_max` function above, we can write a function to plot the `Gini Importance` with a rescaled y-axis reflecting `I` as a proportion of the theoretical maximum:
