...
## Minor
* Line 64: reg.summary(names) only gives first eight!
  + It looks like this is the default behavior coming of `regsubsets()`
    Though you seem to have solved this issue by passing nvmax = 19
* Line 108: All 4 plots stored in variable `reg_plots`???
  + Unfortunately no, you cannot do this while using the grid plotting of
    gridExtra::grid.arrange(). There is a hack by putting them into a list 
    object (as you suggest) but then you have to do some gymnastics to get 
    the grid to work:
    do.call(grid.arrange, reg_plots, ncol = 2). I just didn't feel like explaining
    this complexity, and thought the way it is was simple to understand as it stood.
* Line 201: In the predict function it is tricky to explain what 
  form <- as.formula(object$call[[2]]) is doing. It might be worth commenting.
  + We are just pulling out the model specification (formula) from the original call
    so that we can replicated it internally and use it during prediction.
    This happens under-the-hood in almost every predict() method.
    I have added some additional clarification to the predict() function.
* Line 225: Using for loop instead of purrr ??? (I am sensitized by the issue in lab05)
  + I don't have a problem here, and unfortunately cannot reproduce yours 
    from Lab05. I also don't know where this is any more, since line 225 is no
    longer in the same place! Lets discuss if you feel strongly about it.
    It seems to be fine for now ...
    
  + NOTE: you commented out a ggplot::qplot(), I assumed this was during testing
    and you simply forgot to un-comment it back when you were done, so I 
    did so.
-->
