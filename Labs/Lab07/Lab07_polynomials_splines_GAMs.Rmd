...
pw_fit <- stats::lm(wage ~ cut(age, 4), data = Wage)
broom::tidy(pw_fit)
```  


Visualize the piecewise constant graphically:
```{r pw_plot}
# Define a variable `PieceWise` that groups `age`
# according to the cut values, i.e. the piecewise linear models
tbl <- tibble::tibble(age = age, wage = wage) %>%
  dplyr::mutate(PieceWise = cut(age, 4))
tbl     # quickly view data to be plotted; particularly `PieceWise`
tbl %>% 
  ggplot(aes(x = age, y = wage)) +
  geom_point(alpha = 0.25) +
  geom_smooth(aes(color = PieceWise), method = "lm", se = FALSE) +
  geom_vline(xintercept = c(33.5, 49, 64.5), colour = "blue", linetype = "dashed")
```
