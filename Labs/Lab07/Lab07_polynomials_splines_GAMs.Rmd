...
Default is regression to the logit function which we must transform to obtain probabilities.
...
gam_m3 <- gam::gam(wage ~ s(year, df = 4) + s(age, df = 5) + education,
                   data = Wage)
par(mfrow = c(1, 3))
plot(gam_m2, se = TRUE, col = "purple")
...
# SJT comments 
## Major
* Can we plot the piecewise constant approximation?
* I am not too worried about the difference between the text and the conclusions drawn here 
  + Yeah i suppose, I don't creating it, but I don't think I know what you mean.

## Minor
* Check number of degrees of freedom in spline description
  + Where? Unsure what you mean. You have added df= to many of the spline calls
    which is good. Is that what you mean? I like it, clearer.
* Polynomial logistic regression. Why does upper confidence bound increase so dramatially?
  + I do not know.
* Why echo plot.Gam?
  + Yeah, this was dumb, but I was trying to show that although the gam_ns 
    object was class `lm`, you can plot it as a GAM by calling `plot.Gam()` 
    method if you call it explicitly, bypassing the generic dispatch, so you
    avoid the `plot.lm()` call, which does something different.
    I have since re-coded and we don't actually plot, just mention it.
* When do you use ggplot and when do you use ggplot2?
  + ggplot2 is the package, ggplot() is the function, thus: `ggplot2::ggplot()`
  + Confusing? Yes.
-->
